...
const clamp = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));
  ...
  const getRowXs = (count: number) => {
    const horizontalShift = -2; // Shifting 2% to the left for better visual centering
...
const generateTeamPositions = (team: 'home' | 'away', count: number, sport: Sport = 'football') => {
  if (sport === 'volleyball') {
  ...
  const rows = ROW_SHAPES[clamp(Math.round(count) || 11, 1, 11)];
...
const getInitialPositions = (homeCount: number, awayCount: number, orientation: 'vertical' | 'horizontal', sport: Sport = 'football'): Positions => {
  ...
  if (orientation === 'horizontal') {
    return Object.fromEntries(Object.entries(pos).map(([id, p]) => [id, { x: p.y, y: p.x }]));
  }
  return pos;
};
    ...
    <div className="h-screen bg-bg text-ink selection:bg-teal-100 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside 
          className={`bg-panel border-r border-line overflow-y-auto custom-scrollbar flex flex-col gap-3 transition-all duration-500 ease-in-out fixed lg:relative inset-y-0 left-0 z-50 ${
            sidebarOpen 
              ...
              : 'w-0 -translate-x-full lg:translate-x-0 lg:w-[300px] lg:opacity-100'
        ...
        >
          ...
          <div className="p-4 border-b border-line">
            <h3 className="font-black text-[11px] uppercase tracking-widest text-muted mb-3 flex items-center gap-2">
              <MousePointer2 size={12} /> Etkileşim Modu
            </h3>
            <div className="flex gap-2 p-1 bg-panel-strong dark:bg-panel-strong rounded-lg border border-line">
              <button 
                onClick={() => { setActiveTool('move'); setLinkingPlayerId(null); }}
                className={`flex-1 flex flex-col items-center gap-1 py-2 rounded transition-all ${
                  activeTool === 'move' ? 'bg-accent text-white shadow-md' : 'text-ink/60 hover:bg-black/5 dark:hover:bg-white/5'
                }`}
                title="Hareket Ettir"
              >
                <MousePointer2 size={16} />
                <span className="text-[9px] font-black uppercase">Seç</span>
              </button>
              <button 
                onClick={() => setActiveTool('link')}
                className={`flex-1 flex flex-col items-center gap-1 py-2 rounded transition-all ${
                  activeTool === 'link' ? 'bg-accent text-white shadow-md' : 'text-ink/60 hover:bg-black/5 dark:hover:bg-white/5'
                }`}
                title="Bağla/İşaretle"
              >
                <Link2 size={16} />
                <span className="text-[9px] font-black uppercase">Bağla</span>
              </button>
            </div>
            ...
            {activeTool === 'link' && (
              <motion.div 
                ...
                className="mt-3 p-3 bg-panel-strong dark:bg-panel-strong rounded-lg border border-line space-y-4"
              >
                {/* Color Selection */}
                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-muted mb-2">Renk</p>
                  <div className="flex flex-wrap gap-2">
                    {['#ffffff', colors.homePrimary, colors.awayPrimary, '#fbbf24', '#22c55e'].map(c => (
                      <button
                        key={c}
                        onClick={() => setCurrentLinkStyle(prev => ({ ...prev, color: c }))}
                        className={`w-5 h-5 rounded-full border-2 transition-transform hover:scale-110 ${currentLinkStyle.color === c ? 'border-accent scale-110' : 'border-transparent'}`}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[8px] font-black uppercase tracking-widest text-muted mb-2">Kalınlık</p>
                    <div className="flex gap-1">
                      {[0.4, 0.8].map(t => (
                        <button
                          key={t}
                          onClick={() => setCurrentLinkStyle(prev => ({ ...prev, thickness: t }))}
                          className={`flex-1 h-6 rounded border border-line text-[9px] font-black transition-all ${currentLinkStyle.thickness === t ? 'bg-accent text-white border-accent' : 'text-ink/60 hover:bg-black/5'}`}
                        >
                          {t === 0.4 ? 'İnce' : 'Kalın'}
                        </button>
                      ))}
                  ...
                  <div>
                    <p className="text-[8px] font-black uppercase tracking-widest text-muted mb-2">Çizgi</p>
                    <div className="flex gap-1">
                      {[false, true].map(d => (
                        <button
                          key={String(d)}
                          onClick={() => setCurrentLinkStyle(prev => ({ ...prev, dashed: d }))}
                          className={`flex-1 h-6 rounded border border-line text-[9px] font-black transition-all ${currentLinkStyle.dashed === d ? 'bg-accent text-white border-accent' : 'text-ink/60 hover:bg-black/5'}`}
                        >
                          {d ? 'Kesik' : 'Düz'}
                        </button>
                      ))}
                ...
                {/* Type Selection */}
                <div>
                  <p className="text-[8px] font-black uppercase tracking-widest text-muted mb-2">Görünüm</p>
                  <div className="flex gap-1">
                    {['classic', 'fluid'].map(t => (
                      <button
                        key={t}
                        onClick={() => setCurrentLinkStyle(prev => ({ ...prev, type: t as any }))}
                        className={`flex-1 h-6 rounded border border-line text-[9px] font-black uppercase transition-all ${currentLinkStyle.type === t ? 'bg-accent text-white border-accent' : 'text-ink/60 hover:bg-black/5'}`}
                      >
                        {t === 'classic' ? 'Klasik' : 'Modern'}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {scenes[activeSceneIndex].links && scenes[activeSceneIndex].links!.length > 0 && (
              <button 
                onClick={clearLinks}
                className="w-full mt-2 h-8 text-[10px] uppercase font-black rounded border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2"
              >
                <Trash2 size={12} /> Tüm Bağları Sil
              </button>
            )}
          </div>
          ...
          {/* Settings / Tempo */}
          <div className="p-4 mt-auto border-t border-line bg-panel-strong/20">
             <div className="space-y-4">
                <div className="flex items-center justify-between">
                   <span className="text-[11px] font-black uppercase text-muted">Top Rotası</span>
                   <input 
                    type="checkbox" 
                    checked={showPaths}
                ...
                <div className="space-y-2">
                   <div className="flex justify-between items-center">
                    <span className="text-[11px] font-black uppercase text-muted">Akış Hızı</span>
                    <span className="text-[10px] font-black bg-panel-strong dark:bg-panel-strong px-2 py-0.5 rounded-full border border-line text-ink">{tempo}x</span>
                   </div>
                   <input 
                    type="range" 
                    ...
                    className="w-full accent-accent h-1 bg-line rounded-full appearance-none cursor-pointer"
                   />
                </div>
             </div>
          </div>
        ...
        <main className="flex-1 p-2 sm:p-4 lg:px-8 lg:py-6 flex flex-col gap-3 lg:gap-4 items-center justify-center relative select-none overflow-hidden transition-all duration-500 ease-in-out min-w-0 z-0 text-ink">
          ...
          <div className="w-full max-w-[1400px] flex items-center justify-between transition-all duration-500 gap-3 shrink-0">
            <div className="bg-white dark:bg-panel px-3 lg:px-4 py-1 lg:py-1.5 rounded border border-line shadow-sm shrink-0 min-w-0 max-w-[60%] sm:max-w-none">
                <p className="text-[9px] lg:text-[10px] font-black text-accent uppercase tracking-widest leading-none mb-0.5">Seçili Sahne</p>
                <div className="flex items-center gap-1.5 lg:gap-2">
                  <strong className="text-[11px] lg:text-xs font-black tracking-tight truncate text-ink">{activeSceneIndex + 1}. {scenes[activeSceneIndex].name}</strong>
                  <div className="hidden xs:block h-3 w-px bg-line"></div>
                  <span className="hidden xs:block text-[9px] font-medium text-muted shrink-0 italic">Saha Üzerinde Sürükle</span>
                </div>
             </div>
          </div>
          ...
          >
            ...
            <div 
              ref={pitchRef}
              style={{ 
                aspectRatio: orientation === 'horizontal' ? '3/2' : '2/3',
            ...
            <div className="text-[12px] text-muted italic">
              Oyuncuları ve topu sürükleyerek formasyon oluşturun.
            </div>
          </div>
        </main>
      </div>
